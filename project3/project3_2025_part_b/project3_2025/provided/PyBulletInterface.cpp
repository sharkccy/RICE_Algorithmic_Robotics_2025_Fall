...
public:
    ...
    void startPythonProcess() {
        ...
        if (python_pid_ == 0) {
            ...
            execlp("python3", "python3", "provided/collision_server.py", nullptr);
...
void PyBulletInterface::addBox(const std::vector<double>& half_extents, 
                              const std::vector<double>& position,
                              const std::vector<double>& orientation) {
    if (half_extents.size() != 3 || position.size() != 3 || orientation.size() != 4) {
        std::cerr << "Error: Invalid dimensions for box parameters" << std::endl;
        return;
    }
    
    std::ostringstream cmd;
    cmd << "ADD_BOX";
    for (double ext : half_extents) cmd << " " << ext;
    for (double pos : position) cmd << " " << pos;
    for (double ori : orientation) cmd << " " << ori;
    cmd << "\n";
    
    pimpl_->sendCommand(cmd.str());
    std::string response = pimpl_->readResponse();
    if (response != "OK") {
        std::cerr << "Warning: Failed to add box: " << response << std::endl;
    }
}

void PyBulletInterface::addSphere(double radius, const std::vector<double>& position) {
    if (position.size() != 3) {
        std::cerr << "Error: Invalid position dimensions for sphere" << std::endl;
        return;
    }
    
    std::ostringstream cmd;
    cmd << "ADD_SPHERE " << radius;
    for (double pos : position) cmd << " " << pos;
    cmd << "\n";
    ...
    std::string response = pimpl_->readResponse();
    if (response != "OK") {
        std::cerr << "Warning: Failed to add sphere: " << response << std::endl;
    }
}

void PyBulletInterface::addCylinder(double radius, double height,
                                   const std::vector<double>& position,
                                   const std::vector<double>& orientation) {
    if (position.size() != 3 || orientation.size() != 4) {
        std::cerr << "Error: Invalid dimensions for cylinder parameters" << std::endl;
        return;
    }
    
    std::ostringstream cmd;
    cmd << "ADD_CYLINDER " << radius << " " << height;
    for (double pos : position) cmd << " " << pos;
    for (double ori : orientation) cmd << " " << ori;
    cmd << "\n";
    ...
    std::string response = pimpl_->readResponse();
    if (response != "OK") {
        std::cerr << "Warning: Failed to add cylinder: " << response << std::endl;
    }
}

double PyBulletInterface::getMinimumDistance(const std::vector<double>& config) {
    // Set joint positions first
    setJointPositions(config);
    
    // Request minimum distance
    pimpl_->sendCommand("GET_MIN_DISTANCE\n");
    std::string response = pimpl_->readResponse();
    
    try {
        return std::stod(response);
    } catch (const std::exception& e) {
        std::cerr << "Warning: Failed to get minimum distance: " << response << std::endl;
        return 0.0;
    }
}

void PyBulletInterface::visualizeTrajectory(const std::vector<std::vector<double>>& path, double speed) {
    if (path.empty()) return;
    
    std::ostringstream cmd;
    cmd << "VISUALIZE_TRAJECTORY " << speed << " " << path.size() << "\n";
    
    // Send trajectory data
    for (const auto& config : path) {
        for (size_t i = 0; i < config.size(); ++i) {
            cmd << config[i];
            if (i < config.size() - 1) cmd << " ";
        }
        cmd << "\n";
    }
    cmd << "END_TRAJECTORY\n";
    ...
    std::string response = pimpl_->readResponse();
    if (response != "OK") {
        std::cerr << "Warning: Failed to visualize trajectory: " << response << std::endl;
    }
} 